...
function Starfield() {
    return (
        <div className="absolute inset-0 overflow-hidden">
            ...
            <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-emerald-500/15 rounded-full blur-3xl animate-pulse" />
            <div className="absolute bottom-1/3 left-1/4 w-64 h-64 bg-emerald-400/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
            <svg className="absolute inset-0 w-full h-full opacity-30">
                <defs>
                    <pattern id="dotPattern" width="30" height="30" patternUnits="userSpaceOnUse">
                        <circle cx="2" cy="2" r="1" fill="#34d399" opacity="0.4" />
                    </pattern>
                </defs>
...
function TestimonialCard({ testimonial, isActive }: { testimonial: typeof testimonials[0]; isActive: boolean }) {
    ...
    return (
        <motion.div
            ...
            className={`bg-slate-900/60 backdrop-blur-lg border border-emerald-500/20 rounded-xl p-6 max-w-md ${isActive ? 'shadow-xl shadow-emerald-500/10' : ''
                }`}
        >
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p className="text-emerald-400/80 text-sm">{testimonial.role}</p>
                </div>
            </div>
