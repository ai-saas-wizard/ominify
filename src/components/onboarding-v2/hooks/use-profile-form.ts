...
export function useProfileForm(initialProfile: Record<string, unknown> | null) {
    ...
    // ─── APPLY V2 ANALYSIS RESULT ───

    const applyV2Analysis = useCallback((result: AIAnalysisV2Result) => {
        if (!result.success || !result.profile) return;

        const profile = result.profile;
        setAIOriginalValues(profile as Partial<TenantProfile>);
        ...
        setForm((prev) => {
            ...
            // Ensure business name fields are set
            if (result.businessName) {
                merged.business_description = merged.business_description || result.businessName;
            }
            if (result.industry) {
                merged.industry = merged.industry || result.industry;
            }
            return merged;
        });
