<!--
REFERENCE COPY — the complete script, kept intact.

This is the long-form source of truth: every objection, every CRM name, every
rule, with the reasoning behind them. It is NOT what runs on the assistant.

The deployed prompt is realestatepilotcampaignscript.md, trimmed to the lines
the agent needs mid-call, because a ~30k-character prompt was re-processed by
the model on every single turn and put multi-second gaps into live calls.

When you change policy, change it HERE first, then carry the change into the
trimmed version. Nothing in this file should ever be deleted to save space.
-->
