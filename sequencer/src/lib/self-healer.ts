    ...
    switch (action.type) {
        ...
        case 'inject_fallback_sms': {
            ...
            if (action.details.delay_seconds && step.channel === 'voice') {
                const voiceContent = step.content as VoiceContent;
                if (hasDispatchPlaceholder(voiceContent.first_message) || hasDispatchPlaceholder(voiceContent.system_prompt)) {
                    console.error(`[HEALER] Step ${stepId} voice content is an unrendered dispatch placeholder — not re-queuing call for enrollment ${enrollmentId}`);
                    break;
                }
        ...
        case 'use_alternative_contact': {
            ...
            if (action.details.new_phone) {
                ...
                if (hasDispatchPlaceholder((step.content as SmsContent).body)) {
                    console.error(`[HEALER] Step ${stepId} content is an unrendered dispatch placeholder — skipping alternative-contact SMS for enrollment ${enrollmentId}`);
                    break;
                }
...
/**
 * Wizard-created brief steps store literal "[AI-generated at dispatch]"
 * template content; real copy is generated by the scheduler at dispatch
 * time (intent-guided generation). Healing dispatches reuse raw
 * step.content, so they must never forward the placeholder to a lead —
 * skipping the healing send is always better than sending the marker.
 */
const DISPATCH_PLACEHOLDER = '[AI-generated at dispatch]';

function hasDispatchPlaceholder(text: string | null | undefined): boolean {
    return typeof text === 'string' && text.includes(DISPATCH_PLACEHOLDER);
}
...
async function dispatchToAlternateChannel(
    ...
    switch (newChannel) {
        case 'sms': {
            ...
            if (hasDispatchPlaceholder(body)) {
                console.error(`[HEALER] Step ${stepId} content is an unrendered dispatch placeholder — skipping alternate SMS for enrollment ${enrollmentId}`);
                return;
            }
            await smsQueue.add('sms:send', {
                tenantId,
        ...
        case 'email': {
            ...
            if (hasDispatchPlaceholder(textContent)) {
                console.error(`[HEALER] Step ${stepId} content is an unrendered dispatch placeholder — skipping alternate email for enrollment ${enrollmentId}`);
                return;
            }
